            ...
            <div className="flex flex-1 flex-col items-end justify-end gap-1 pb-1">
              <div className="flex items-center rounded-md border border-input bg-transparent shadow-sm">
                <Button
                  ...
                  size="sm"
                  className={cn(
                    "h-6 w-8 rounded-none rounded-l-md px-1",
                    activeTarget.visible ? "bg-accent" : ""
                  )}
                ...
                <Button
                  ...
                  size="sm"
                  className={cn(
                    "h-6 w-8 rounded-none rounded-r-md px-1",
                    !activeTarget.visible ? "bg-accent" : ""
                  )}
            ...
            <div className="mt-1 flex items-center gap-2">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-xs font-medium">
                  {stage.costumes[stage.currentCostume].name}
                </span>
              </div>
            </div>
          )}
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="relative flex flex-1 flex-col border-r border-border">
          ...
          <ScrollArea className="flex-1">
            <div className="grid grid-cols-2 gap-2 p-2 pb-12 sm:grid-cols-3">
              {sprites.map((sprite) => (
                <div
                  key={sprite.id}
                ...
                >
                  <div className="mb-2 flex h-10 w-10 items-center justify-center rounded-full bg-muted">
                    ...
                    <div
                      ...
                      style={{
                        backgroundColor: sprite.name.toLowerCase().includes("ball")
                          ? "yellow"
                          : "orange",
                      }}
                    />
          ...
          <Button
            size="icon"
            className="absolute bottom-4 right-4 z-50 h-10 w-10 rounded-full shadow-lg"
        ...
        <div className="relative flex w-32 flex-col border-l border-border bg-background">
          <div className="flex items-center justify-between border-b border-border bg-muted/20 p-2">
            <span className="text-xs font-semibold uppercase text-muted-foreground">Backdrops</span>
            <span className="text-[10px] text-muted-foreground">
              {stage?.costumes?.length || 0}
            </span>
          </div>
            ...
            <div className="flex flex-col gap-2 p-2 pb-12">
              {stage?.costumes?.map((costume, index) => (
                <div
                  key={costume.id}
                  className={cn(
                    "group relative flex cursor-pointer flex-col items-center gap-1 rounded-md border p-1 transition-colors hover:bg-muted/50",
                    activeTargetId === stage.id && stage.currentCostume === index
                ...
                >
                  <div className="relative aspect-[4/3] w-full overflow-hidden rounded border border-border bg-white shadow-sm">
                    <div
          ...
          <Button
            size="icon"
            className="absolute bottom-4 right-4 z-50 h-8 w-8 rounded-full shadow-lg"
        ...
        <DialogContent className="max-w-2xl">
          ...
          <Tabs defaultValue={pickerType} className="w-full">
            <TabsList className="mb-4">
              <TabsTrigger value="sprite" onClick={() => setPickerType("sprite")}>
                Sprites
              </TabsTrigger>
              <TabsTrigger value="backdrop" onClick={() => setPickerType("backdrop")}>
                Backdrops
              </TabsTrigger>
            </TabsList>
              ...
              <ScrollArea className="h-full">
                <div className="grid grid-cols-4 gap-4 p-2">
                  {SPRITE_ASSETS.map((asset, i) => (
                    <div
                      key={i}
                      className="flex cursor-pointer flex-col items-center gap-2 rounded-md border p-4 hover:bg-muted"
              ...
              <ScrollArea className="h-full">
                <div className="grid grid-cols-3 gap-4 p-2">
                  {BACKDROP_ASSETS.map((asset, i) => (
                    <div
                      key={i}
                      className="flex cursor-pointer flex-col gap-2 rounded-md border p-2 hover:bg-muted"
                      onClick={() => handleAssetSelect(asset)}
                    >
                      <div
                        ...
                        style={{ backgroundColor: asset.color }}
                      />
                      <span className="text-center text-sm font-medium">{asset.name}</span>
                    </div>
                  ))}
                </div>
              </ScrollArea>
